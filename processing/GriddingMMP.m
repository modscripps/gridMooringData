...
for idx_var = 1 : length(FP.VarNameList)
    ...
    TEMP = nan( length(MMP.z), length(MMP.yday_grid) );

    %% 
    %for idx_z = 1 : length(MMP.z)
    for idx_z=1:size(MMP.z,1)
        data = MMP.(varname)(idx_z, :);
    ...
    for  idx_yday = 1 : size( MMP.yday_grid,2 )
        ...
        elseif length(z) == 1
            ...
            data_z_grid = interp1( z, data, MMP.z_grid, [] );
            MMP.([varname '_grid'])(:, idx_yday) = data_z_grid;
