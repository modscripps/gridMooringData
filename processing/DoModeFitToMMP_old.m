...
%function MMP = DoModeFitToMMP(MMP, FP)
%   decompose into discrete modes
%
%   INPUT:
%       a set of modes, defined over the whole water depth
%       MMP.z_full:         vertical grid
%       MMP.vert_full:      vertical modes (eta, w)
%       MMP.hori_full:      horizontal modes (u, v)
%
%       data fields
...
for wh = 1 : LENGTH
    ...
    for idx_var = 1 : 3  
        ...
        %% check number of good layers 
        Num_gd_layer = length( find( any( ~isnan(MMP.(varname)), 2) ) );
        ...
        if strcmp( varname, 'u' ) | strcmp( varname, 'v')
            I = ones(size(data));
            G = [G I];
        end

        %% check bad data per profile
        idx_bd = find( isnan(data) );
        data(idx_bd) = [];   % get rid of nan data
        G(idx_bd, :) = [];
            
        if length(data) > FP.good_percent * Num_gd_layer
