...
for indvar = 1 : length(GP.VarNameList)
    
    varname = GP.VarNameList{indvar};
    disp(['    Working on variable: ' varname])
    
    % Pre-allocate space for the gridded indvar'th variable:
    griddedDATA.([varname '_grid']) = NaN(length(griddedDATA.z_grid), ...
                                          length(griddedDATA.yday_grid));
    
	% Pre-allocate space for an array that is the result of the first
    % iteration of the loop below. If we first grid in depth, then
    % auxsize has the same number of rows as z_grid and the same number
    % of columns (yday) as the data:
    if GP.rcinterp(1)==1
        auxsize = [length(griddedDATA.z_grid), length(griddedDATA.yday)];
    else
        auxsize = [length(griddedDATA.z), length(griddedDATA.yday_grid)];
    end
    ...
    for idim = 1:length(GP.rcinterp)
        ...
        % Deal with gridding in time:
        if GP.rcinterp(idim)==2
            
            nz = size(griddedDATA.z, 1);   % gotta make sure that point
                                           % measurements corrected by
                                           % knockdown have a griddedDATA.z
                                           % as COLUMN VECTOR!!!!
                                                       
            if isvector(griddedDATA.(GP.OriginalTime))
                indyday = ones(1, nz);
            else
                indyday = 1:nz;
            end
            ...
            for idx_z = 1:nz
                ...
                yday = griddedDATA.(GP.OriginalTime)(indyday(idx_z), :);
                data = griddedDATA.(varname)(idx_z, :);
                
                % Subset for non-NaN data points:
                lgoodata  = ~isnan(yday) & ~isnan(data);
                yday = yday(lgoodata);
                data = data(lgoodata);
                ...
                elseif length(yday) == 1
                    ...
                    % ------------------------------------------------
                    % DAMN CASE WHERE WE ONLY HAVE ONE YDAY!!!!
                    % ------------------------------------------------
                    
                else
                    
                    % First compute the time step of the time grid
                    % (as far as OBM knows, yday_grid should always be
                    % regular, but I'll keep this more general nanmean):
                    DT = nanmean(diff(griddedDATA.yday_grid));
                    
                    % First check what are the grid points that are close
                    % to enough the data:
                    [~, distFromData] = dsearchn(griddedDATA.yday_grid(:), ...
                                                                  yday(:));             
                    lclose = distFromData <= (DT * GP.MaxTimesDT);
                    
                    % Finally, interpolate data in time:
                    data_yday_grid = interp1(yday, data, ...
                                            griddedDATA.yday_grid(lclose));
                    
                    % Assign interpolated variable to
                    % the previously created array:
                    auxVar(idx_z, lclose) = data_yday_grid;
                    
                end
 
            end
            
            % If it is an instrument at a single depth and its depth
            % is given at every time stamp, also grid the variable
            % depth of the instrument in time:
            if nz==1 && length(griddedDATA.z)>1
                griddedDATA.z = interp1(yday(:),          ...
                                        griddedDATA.z(:), ...
                                        griddedDATA.yday_grid(:));
                % griddedDATA.z probably needs to be a column vector at
                % this point
            end
            
        % Deal with gridding in depth (which may not be gridding :
        elseif GP.rcinterp(idim)==1
                                
            for i = (MMP.yday_grid, 2)
...
% % %         % When gridding in depth, we must loop over columns. When gridding
% % %         % in time, we loop over rows. So if GP.rcinterp(idim) is 1 (2) we
% % %         % now loop over columns (rows):
% % %         if GP.rcinterp(idim)==1
% % %             var2loopover = ;
% % %         elseif GP.rcinterp(idim)==2
% % %             var2loopover = ;
% % %         end
